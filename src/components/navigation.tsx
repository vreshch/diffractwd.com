    ...
    >
      <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          ...
          {navItems.map(({ href, label }) => {
            ...
            return (
              ...
              >
                {label}
                <span
                  className={`absolute inset-x-3 -bottom-[1px] h-[2px] rounded-full transition-all duration-300 ${
                    isActive
        ...
        <div className="flex items-center gap-1 md:hidden">
          <ThemeToggle />
          <button
            className="relative rounded-lg p-2 text-muted transition-colors duration-200 hover:bg-surface-hover hover:text-heading dark:text-dark-text-secondary dark:hover:bg-dark-surface dark:hover:text-dark-text md:hidden"
          ...
          >
            <div className="relative h-5 w-5">
              ...
              />
              <span
                className={`absolute left-0 top-[9px] block h-[2px] w-5 rounded-full bg-current transition-all duration-200 ${
          ...
          {navItems.map(({ href, label }) => {
            ...
            return (
              <Link
                key={href}
                href={href}
                className={`relative block rounded-lg px-3 py-2.5 text-sm font-medium transition-all duration-200 ${
                  isActive
