...
function FaviconVariantsSection() {
  return (
    <Section
      id="favicon"
      title="App Icon"
      subtitle="Overlay pattern — solid top trace + half-opacity bottom trace (32×32 native, 5× zoom)"
    >
...
function GradientBox({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="rounded-xl bg-gradient-to-br from-primary to-primary-light p-8">
      <p className="mb-5 text-xs font-semibold uppercase tracking-wider text-white/70">{label}</p>
      <div className="space-y-5">{children}</div>
    </div>
  );
}
...
function ButtonsSection() {
  return (
    <Section
      id="buttons"
      title="Buttons"
      subtitle="Primary and secondary actions on surface and gradient backgrounds"
    >
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <ShowcaseBox label="On Surface">
          <Row>
            ...
            <span className="inline-block rounded-full bg-primary px-3 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-white dark:bg-accent">
              Recommended
            </span>
          </Row>
          <Row>
            <span className="inline-block rounded-md bg-primary/12 px-2.5 py-0.5 text-xs font-semibold text-primary dark:bg-accent/10 dark:text-accent-light">
              v1.30
            </span>
            <span className="inline-block rounded-md bg-primary/6 px-2.5 py-0.5 text-xs font-semibold text-primary/60 dark:bg-dark-surface-alt dark:text-dark-text-secondary">
              v1.20
            </span>
            <span className="inline-block rounded-md bg-primary/6 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-primary/60 dark:bg-dark-surface-alt dark:text-dark-text-secondary">
              .EXE
            </span>
            <span className="inline-block rounded-md bg-primary/6 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-primary/60 dark:bg-dark-surface-alt dark:text-dark-text-secondary">
              .ZIP
        ...
        <div className="rounded-xl border border-border bg-white p-6 dark:border-dark-border dark:bg-dark-surface">
          ...
          <ul className="mt-3 space-y-2 text-sm text-body dark:text-dark-text">
            <li className="flex items-center gap-2">
              <span className="text-primary dark:text-accent-light">&#8226;</span>Bullet item one
            </li>
            <li className="flex items-center gap-2">
              <span className="text-primary dark:text-accent-light">&#8226;</span>Bullet item two
            </li>
            <li className="flex items-center gap-2">
              <span className="text-primary dark:text-accent-light">&#8226;</span>Bullet item three
            </li>
          </ul>
        </div>
      </div>
...
function IconsSection() {
  return (
    <Section
      ...
      subtitle="Tinted background containers at three sizes"
    >
      <ShowcaseBox label="Sizes">
        <Row>
          <div className="flex items-center gap-6">
            ...
            <div className="text-center">
              <div className="inline-flex h-10 w-10 items-center justify-center rounded-lg bg-primary/10 text-primary dark:bg-accent/10 dark:text-accent-light">
                <CheckIcon />
              </div>
              <p className="mt-2 text-xs text-muted dark:text-dark-text-secondary">40px</p>
            </div>
            <div className="text-center">
              <div className="inline-flex h-12 w-12 items-center justify-center rounded-xl bg-primary/10 text-primary dark:bg-accent/10 dark:text-accent-light">
                <CheckIcon />
              </div>
    ...
    <Section id="typography" title="Typography" subtitle="Headings, body text, labels, and links">
      <ShowcaseBox label="Headings & Text">
        <div className="space-y-6">
          <div>
            <h1 className="text-4xl font-bold tracking-tight text-heading dark:text-dark-text md:text-5xl">
              Page Heading h1
            </h1>
          </div>
          <div>
            <h2 className="text-3xl font-bold tracking-tight text-heading dark:text-dark-text">
              Section Heading h2
            </h2>
          </div>
          <div>
            <h3 className="text-2xl font-bold tracking-tight text-heading dark:text-dark-text">
              Subsection h3
            </h3>
          </div>
          <div>
          ...
          <div>
            <p className="text-base leading-relaxed text-muted dark:text-dark-text-secondary">
              Body text — used for descriptions, paragraphs, and general content throughout the
              site.
            </p>
          </div>
          <div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <ShowcaseBox label="On Surface">
          <Row>
            <a
              href="#"
              className="inline-flex items-center gap-1.5 text-sm font-medium text-accent hover:underline dark:text-accent-light"
            >
              <ExternalLinkIcon />
            ...
            {rows.map((r) => (
              <tr key={r.prop} className="border-t border-border dark:border-dark-border">
                ...
                <td className="px-6 py-3.5">
                  <span
                    className={`inline-block rounded-md px-2.5 py-0.5 text-xs font-semibold ${r.active ? 'bg-primary/12 text-primary dark:bg-accent/10 dark:text-accent-light' : 'bg-primary/5 text-primary/60 dark:bg-dark-surface-alt dark:text-dark-text-secondary'}`}
                  >
                    {r.status}
                  </span>
                </td>
